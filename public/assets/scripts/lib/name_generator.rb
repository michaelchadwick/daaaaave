...
class NameGenerator
  attr_accessor :type, :variety, :name_set, :chain_cache

  def initialize(type, variety)
    self.type         = type
    self.variety      = variety
    self.name_set     = get_name_set(self.type, self.variety)
    self.chain_cache  = {}
  end

  # load sample names
  def get_name_set(type, variety)
    names = []
    names_data = YAML.load_file(
      File.join(__dir__, "corpus/#{type}/#{variety}.yaml")
    )
